...
use crate::segy::ByteOrder;
use crate::spec::types::HeaderFieldSpec;
...
pub use crate::segy::ByteOrder as HeaderByteOrder;
...
pub enum ScalarType {
    ...
    Float32,
    Float64,
    /// IBM 360/370 32-bit floating point. Technically not a header field type
    /// per the SEG-Y standard, but accepted because some custom specs use it.
    /// Value is returned as the raw bit pattern (i64); use the resolved string
...
impl FromStr for ScalarType {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s.to_lowercase().as_str() {
            ...
            "ibm32" | "ibmfloat" | "ibm" => Ok(Self::Ibm32),
            _ => Err(()),
        }
    }
}
...
impl ScalarType {
    pub fn size(&self) -> usize {
        match self {
            ...
            Self::Int32 | Self::Uint32 | Self::Float32 | Self::Ibm32 => 4,
            Self::Int64 | Self::Uint64 | Self::Float64 => 8,
        }
    }

    /// Canonical string key (matches the spec JSON / TS `DATA_TYPES` list).
    ...
    /// Human-readable display name. Unsigned variants append " (unsigned)".
    pub fn display_name(&self) -> &'static str {
        match self {
            Self::Int8 => "8-bit integer",
            Self::Uint8 => "8-bit integer (unsigned)",
            Self::Int16 => "16-bit integer",
            Self::Uint16 => "16-bit integer (unsigned)",
            Self::Int32 => "32-bit integer",
            Self::Uint32 => "32-bit integer (unsigned)",
            Self::Int64 => "64-bit integer",
            Self::Uint64 => "64-bit integer (unsigned)",
            Self::Float32 => "32-bit float",
            Self::Float64 => "64-bit float",
            Self::Ibm32 => "IBM 32-bit float",
        }
    }
...
pub struct HeaderSpec {
    pub size: u16,
    pub fields: HashMap<String, (usize, ScalarType), BuildHasherDefault<DefaultHasher>>,
...
impl HeaderSpec {
    pub fn from_fields(fields: Vec<HeaderField>, size: u16) -> Self {
        ...
        for (index, field) in fields.iter().enumerate() {
            ...
            map.insert(field.field_key.clone(), (index, field.format));
        }
        Self {
            size,
            fields: map,
    ...
    pub fn from_specs(specs: &[HeaderFieldSpec], size: u16) -> Result<Self, HeaderError> {
        let mut fields: Vec<_> = specs.iter().filter_map(HeaderField::from_spec).collect();
...
impl<'a> RuntimeHeaderView<'a> {
    ...
    pub fn get_raw(&self, field: &HeaderField) -> Result<i64, HeaderError> {
        ...
        let value = match field.format {
            ...
            // IBM float is always big-endian in SEG-Y; return the raw bit pattern.
            ScalarType::Ibm32 => BigEndian::read_u32(bytes) as i64,
        };
...
mod tests {
    ...
    fn test_revision_field_count_differs() {
        ...
        let rev1_fields = vec![
            ...
            HeaderField {
                name: "Inline Number".to_string(),
                field_key: "inline_number".to_string(),
                byte: 181,
                format: ScalarType::Int32,
            },
            HeaderField {
                name: "Crossline Number".to_string(),
                field_key: "crossline_number".to_string(),
                byte: 185,
                format: ScalarType::Int32,
            },
        ];
